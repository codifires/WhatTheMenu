...
// @desc    PhonePe Subscription Payment Callback
// @route   POST /api/owner/subscription/payment-callback
const subscriptionPaymentCallback = async (req, res) => {
  try {
    ...
    if (providedChecksum === calculatedChecksum) {
      const subRequest = await SubscriptionRequest.findOne({ utr_number: parsedResponse.data.merchantTransactionId });
      
      if (subRequest && subRequest.status === 'pending') {
        if (parsedResponse.success && parsedResponse.code === 'PAYMENT_SUCCESS') {
          ...
          if (cafe.subscription?.end_date && new Date(cafe.subscription.end_date) > now && cafe.subscription.status === 'active') {
            ...
            // Clear any upcoming subscription
            cafe.upcoming_subscription = undefined;
            ...
            await Subscription.findOneAndUpdate(
              ...
              {
                plan_name: subRequest.plan_name,
                price: subRequest.amount || (subRequest.plan_name === 'pro' ? 499 : 299),
                start_date: new Date(),
                end_date: endDate,
                status: 'active'
              },
...
// @desc    Verify PhonePe Subscription Payment Status (Frontend Polling)
// @route   POST /api/owner/subscription/verify
const verifySubscriptionPayment = async (req, res, next) => {
  try {
    ...
    const subRequest = await SubscriptionRequest.findOne({ utr_number: transactionId, cafe_id: req.user._id });
    if (!subRequest) {
      return res.status(404).json({ success: false, message: 'Subscription request not found' });
    }

    // If already approved (maybe by webhook), return success immediately
    if (subRequest.status === 'approved') {
    ...
    // Use test URL by default for status checking
    const statusApiUrl = `https://api-preprod.phonepe.com/apis/pg-sandbox${endpoint}`;
    ...
    try {
      ...
      if (response.data && response.data.success && response.data.code === 'PAYMENT_SUCCESS') {
        ...
        if (cafe.subscription?.end_date && new Date(cafe.subscription.end_date) > now && cafe.subscription.status === 'active') {
          // Current plan is still active, queue the new plan
          cafe.upcoming_subscription = {
            plan_name: subRequest.plan_name,
            duration_days: 30
          };
        } else {
          // No active plan or plan expired, activate immediately
          const endDate = new Date();
          endDate.setDate(endDate.getDate() + 30);
      ...
      return res.status(500).json({ success: false, message: 'Failed to verify payment status with PhonePe' });
    }
