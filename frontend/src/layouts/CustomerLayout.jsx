...
const CustomerLayout = ({ children }) => {
  ...
  const navItems = [
    { to: `/menu/${id}`, label: 'Menu', end: true, icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/>
        <path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/>
      </svg>
    )},
    { to: `/menu/${id}/search`, label: 'Search', icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
        <circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
      </svg>
    )},
    { to: `/menu/${id}/cart`, label: 'Cart', badge: totalItems, icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
    ...
    { to: `/menu/${id}/orders`, label: 'Orders', icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
    ...
    { to: `/menu/${id}/feedback`, label: 'Feedback', icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
    ...
    <div style={{ minHeight: '100vh', background: '#0a0d14', color: 'var(--text-primary)', paddingBottom: isCheckout ? 0 : 100, maxWidth: 500, margin: '0 auto', position: 'relative', fontFamily: "'Inter',sans-serif" }}>
      ...
      {!isCheckout && (
        <nav style={{ position: 'fixed', bottom: 20, left: 16, right: 16, zIndex: 100, display: 'flex', justifyContent: 'center' }}>
          <div style={{
            width: '100%', maxWidth: 440, background: 'rgba(20,24,35,0.85)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)',
