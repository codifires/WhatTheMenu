  ...
  return (
    <NavLink
      ...
      style={({ isActive }) => ({
        ...
        padding: '12px 16px', borderRadius: '0 24px 24px 0',
        textDecoration: 'none', fontSize: 15, fontWeight: 500,
        transition: 'all 0.2s ease',
        color: isActive ? 'var(--text-primary)' : 'var(--text-tertiary)',
        background: isActive ? 'linear-gradient(90deg, rgba(6, 182, 212, 0.15) 0%, rgba(6, 182, 212, 0) 100%)' : 'transparent',
        borderLeft: isActive ? '4px solid #06b6d4' : '4px solid transparent',
        marginLeft: '-1px'
      })}
      onMouseEnter={e => { if (!e.currentTarget.style.borderLeft.includes('06b6d4')) { e.currentTarget.style.color = 'var(--text-primary)'; e.currentTarget.style.background = 'var(--bg-card-hover)'; }}}
      onMouseLeave={e => { if (!e.currentTarget.style.borderLeft.includes('06b6d4')) { e.currentTarget.style.color = 'var(--text-tertiary)'; e.currentTarget.style.background = 'transparent'; }}}
    >
      {link.icon}
      {link.label}
    </NavLink>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ padding: '32px 24px', display: 'flex', alignItems: 'center', gap: 14 }}>
        <div style={{ width: 44, height: 44, borderRadius: 12, background: 'linear-gradient(135deg,#4f46e5,#06b6d4)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#FFF', flexShrink: 0, boxShadow: '0 4px 16px rgba(6,182,212,0.3)' }}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 8h1a4 4 0 0 1 0 8h-1"></path><path d="M2 8h16v9a4 4 0 0 1-4 4H6a4 4 0 0 1-4-4V8z"></path><line x1="6" y1="1" x2="6" y2="4"></line><line x1="10" y1="1" x2="10" y2="4"></line><line x1="14" y1="1" x2="14" y2="4"></line></svg>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <h1 style={{ margin: 0, color: 'var(--text-primary)', fontSize: 20, fontWeight: 800, fontFamily: "'Outfit', sans-serif", whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {user?.name || 'My Café'}
          </h1>
          <p style={{ margin: '2px 0 0', color: '#06b6d4', fontSize: 10, fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase' }}>Owner Portal</p>
        </div>
      </div>

      {/* Navigation */}
      <nav style={{ flex: 1, padding: '0', display: 'flex', flexDirection: 'column', gap: 24, overflowY: 'auto' }}>
        {NAV_GROUPS.map((group, idx) => (
          <div key={idx}>
            <div style={{ padding: '0 24px', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-tertiary)', letterSpacing: '1.5px', textTransform: 'uppercase' }}>{group.title}</span>
              {idx > 0 && <div style={{ flex: 1, height: 1, background: 'var(--border-light)' }}></div>}
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, paddingBottom: 24, marginTop: 'auto' }}>
          <div style={{ height: 1, background: 'var(--border-light)', margin: '0 24px 12px 24px' }}></div>
          <button
            onClick={onLogout}
            style={{
              display: 'flex', alignItems: 'center', gap: 14,
              ...
              borderLeft: '4px solid transparent', marginLeft: '-1px'
            }}
            onMouseEnter={e => { e.currentTarget.style.color = 'var(--danger-text)'; e.currentTarget.style.background = 'var(--danger-light)'; }}
...
const OwnerLayout = ({ children }) => {
  ...
  const pageTitle = allLinks.find(l => l.end ? location.pathname === l.to : location.pathname.startsWith(l.to))?.label || 'Dashboard'
      ...
      <div style={S.main}>
        <header style={S.topbar} className="owner-topbar">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 12, color: 'var(--text-tertiary)', fontWeight: 500 }}>Caf├⌐</span>
        ...
        <main style={S.content} className="owner-content">
          {isRestricted ? (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '40px 20px', minHeight: '60vh', animation: 'fadeIn 0.5s ease' }}>
              <div style={{ fontSize: 80, marginBottom: 24, filter: 'drop-shadow(0 10px 20px rgba(0,0,0,0.2))' }}>≡ƒöÆ</div>
