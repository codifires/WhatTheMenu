    ...
    <div style={{ padding: '20px 16px', animation: 'fadeIn 0.4s ease' }}>
      ...
      <div style={{ marginBottom: 24, position: 'sticky', top: 0, paddingTop: 10, paddingBottom: 10, background: 'var(--bg-main)', zIndex: 10 }}>
        ...
        <div style={{ position: 'relative' }}>
          <svg style={{ position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-secondary)' }} width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
          ...
          <input
            ...
            style={{
              ...
              background: 'var(--bg-input)', color: 'var(--text-primary)', fontSize: 16, fontWeight: 600, outline: 'none',
            ...
            onBlur={e => { e.target.style.borderColor = 'var(--border-hover)'; e.target.style.background = 'var(--bg-input)' }}
          />
        </div>
      </div>
        ...
        {results.map((item, i) => {
          ...
          return (
            <div key={item._id} style={{
              display: 'flex', alignItems: 'center', gap: 12, padding: 12, borderRadius: 20, background: 'var(--bg-card)',
              border: '1px solid rgba(255,255,255,0.05)', animation: `slideUp 0.3s ease ${i * 0.05}s both`
            }}>
              
              {/* Image Box */}
              <div style={{ width: 70, height: 70, borderRadius: 12, background: 'var(--bg-card)', position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
                {item.image ? (
                ...
                <div style={{ position: 'absolute', top: 4, left: 4, width: 10, height: 10, borderRadius: 3, background: 'var(--text-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 1 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ fontSize: 12, color: 'var(--text-tertiary)', margin: '0 0 4px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.category_id?.name || 'Item'}</p>
                <p style={{ fontSize: 14, fontWeight: 800, color: item.availability === false ? 'var(--text-tertiary)' : '#f59e0b', margin: 0 }}>₹{item.price}</p>
              </div>

              {/* Actions */}
              <div style={{ flexShrink: 0 }}>
                {item.availability === false ? (
                  <span style={{ padding: '4px 8px', borderRadius: 8, background: 'var(--border-light)', color: 'var(--text-secondary)', fontSize: 10, fontWeight: 700 }}>
                    Out of Stock
                  </span>
                ) : qty > 0 ? (
      ...
      {query.length >= 2 && results.length === 0 && !searching && (
        <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--text-tertiary)', animation: 'fadeIn 0.3s' }}>
          <span style={{ fontSize: 40, display: 'block', marginBottom: 12, opacity: 0.5 }}>🤷</span>
          <p style={{ fontSize: 16, fontWeight: 600, color: 'var(--text-primary)', margin: '0 0 4px' }}>No items found</p>
