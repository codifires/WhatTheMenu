...
const STATUS_COLORS = {
  ...
  cancelled: { bg: 'rgba(245,158,11,0.1)', color: '#fbbf24', border: 'rgba(245,158,11,0.2)' }
}
...
const AdminPayments = () => {
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", color: '#fff' }}>
      ...
      <div style={{ borderRadius: 20, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', overflow: 'hidden', animation: 'slideUp 0.5s ease' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 800 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                {['Café Details', 'Plan Type', 'Amount', 'Date', 'Transaction ID', 'Status'].map((h, i) => (
                  <th key={h} style={{ padding: '14px 20px', fontSize: 11, fontWeight: 700, color: '#4b5563', textAlign: 'left', letterSpacing: 1, textTransform: 'uppercase', whiteSpace: 'nowrap', background: 'rgba(255,255,255,0.01)' }}>
                    {h}
                  </th>
                ))}
              </tr>
              ...
              {loading ? (
                    ...
                    <tr key={payment._id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', transition: 'background 0.15s' }}
                      ...
                      <td style={{ padding: '16px 20px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                          ...
                          <div>
                            <p style={{ fontSize: 14, fontWeight: 600, color: '#e5e7eb', margin: 0, whiteSpace: 'nowrap' }}>{payment.cafe_id?.name || 'Unknown Café'}</p>
                            <p style={{ fontSize: 11, color: '#6b7280', margin: '2px 0 0' }}>{payment.cafe_id?.email || 'N/A'}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      {/* Plan Type */}
                      <td style={{ padding: '16px 20px' }}>
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 600, color: isPro ? '#f59e0b' : '#a78bfa', padding: '4px 10px', borderRadius: 6, background: isPro ? 'rgba(245,158,11,0.1)' : 'rgba(124,58,237,0.1)', textTransform: 'capitalize' }}>
                      ...
                      {/* Date */}
                      <td style={{ padding: '16px 20px', fontSize: 13, color: '#d1d5db' }}>
                        {new Date(payment.created_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}
                      </td>

                      {/* Transaction ID */}
                      <td style={{ padding: '16px 20px', fontSize: 12, fontFamily: 'monospace', color: '#9ca3af' }}>
                ...
                <tr>
                  <td colSpan={6} style={{ padding: '60px 20px', textAlign: 'center' }}>
                    ...
                    <p style={{ fontSize: 15, fontWeight: 600, color: '#6b7280', margin: '0 0 6px' }}>No payments found</p>
                    <p style={{ fontSize: 13, color: '#374151', margin: 0 }}>Subscription payments will appear here.</p>
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', padding: '16px 20px', borderTop: '1px solid rgba(255,255,255,0.05)', gap: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              <button 
                ...
                disabled={page === 1}
                style={{ padding: '6px 12px', borderRadius: 6, background: page === 1 ? 'transparent' : 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: page === 1 ? '#4b5563' : '#fff', cursor: page === 1 ? 'not-allowed' : 'pointer' }}
              >
                Previous
              </button>
              <button 
                ...
                style={{ padding: '6px 12px', borderRadius: 6, background: page === totalPages ? 'transparent' : 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: page === totalPages ? '#4b5563' : '#fff', cursor: page === totalPages ? 'not-allowed' : 'pointer' }}
              >
                Next
              </button>
            </div>
